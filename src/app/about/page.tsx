      ...
      <main className="pt-16">
        ...
        <section className="max-w-6xl mx-auto px-6 md:px-12 py-20 md:py-28">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <p className="text-[color:var(--text-secondary)] text-lg leading-relaxed mt-5">
              ...
              <div className="mt-8">
                <Link
                  href="/register"
                  className="inline-flex items-center gap-2 btn-gold px-6 py-3 rounded-xl text-sm font-semibold transition-all"
                >
              ...
              {values.map(({ icon: Icon, label, desc }) => (
                <MagicCard key={label} className="rounded-xl">
                  <div className="p-5 h-full flex flex-col gap-3">
                    ...
                    <p className="text-[color:var(--text-secondary)] text-sm font-semibold leading-tight">
                      {label}
                    </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-0 md:divide-x md:divide-white/[0.06]">
              {stats.map(({ value, label }) => (
                ...
                >
                  ...
                  <span className="text-[color:var(--text-tertiary)] text-sm tracking-wide">
                    {label}
                  </span>
                </div>
              ))}
        ...
        <section className="max-w-6xl mx-auto px-6 md:px-12 py-20 md:py-28">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <h2 className="font-serif text-[32px] leading-tight tracking-[-0.015em] text-[color:var(--text-primary)] md:text-[40px] mb-8 leading-snug">
        ...
        <section className="max-w-6xl mx-auto px-6 md:px-12 py-20 md:py-28">
          <div className="mb-14 text-center">
            ...
            <p className="text-[color:var(--text-tertiary)] text-sm mt-3 max-w-md mx-auto">
            ...
            {team.map(({ initials, name, role, bio }) => (
              <AnimatedBorder key={name}>
                <div className="p-6 flex flex-col gap-4">
                  <div className="w-12 h-12 rounded-full bg-[color:var(--surface-1)] border border-vodium-gold/40 flex items-center justify-center">
                    <span className="font-serif text-vodium-gold text-lg leading-none">
                      {initials}
                    </span>
                  </div>
                  <div>
                    <p className="text-[color:var(--text-primary)] font-semibold text-sm leading-tight">
                      {name}
                    </p>
        ...
        {/* What's next */}
        <section className="bg-[color:var(--surface-1)] border-y border-[color:var(--hairline)]">
          <div className="max-w-6xl mx-auto px-6 md:px-12 py-20">
            <div className="grid md:grid-cols-2 gap-10 items-center">
              <div>
                ...
                <p className="text-[color:var(--text-secondary)] text-base leading-relaxed mb-5">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div
                    key={label}
                    className="flex items-start gap-4 p-4 rounded-xl bg-white/[0.02] border border-[color:var(--hairline)]"
                  >
                    <div
                      className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${i === 0 ? "bg-vodium-gold/15 border border-vodium-gold/30" : "bg-white/[0.04] border border-[color:var(--hairline)]"}`}
                    >
                      <Icon
                        size={15}
                        className={
                          i === 0 ? "text-vodium-gold" : "text-[color:var(--text-quaternary)]"
                        }
                      />
                    </div>
                    <div>
                      <p
                        className={`text-sm font-semibold leading-tight ${i === 0 ? "text-[color:var(--text-primary)]" : "text-[color:var(--text-tertiary)]"}`}
                      >
                        {label}
                      </p>
                      <p className="text-[color:var(--text-quaternary)] text-xs mt-1 leading-relaxed">
                        {desc}
                      </p>
                    </div>
                  </div>
        ...
        <section className="max-w-6xl mx-auto px-6 md:px-12 py-20 text-center">
          <h2 className="font-serif text-3xl text-[color:var(--text-primary)] mb-4">
            Get in touch.
          </h2>
          <p className="text-[color:var(--text-tertiary)] mb-8 max-w-md mx-auto">
            Partnerships, press, investors, or just want to learn more about
            what we&apos;re building?
          </p>
          <a
            href="mailto:hello@vodiumledger.com"
            className="inline-flex items-center gap-2.5 btn-gold px-7 py-3.5 rounded-xl text-sm font-semibold transition-all"
          >
            <Mail size={15} />
