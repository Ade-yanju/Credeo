 ...
 *
 * Reports are template-only, just like invoices: a missing or unapproved
 * template is reported as undelivered and never downgraded to plain text.
 */

import { formatNaira } from "@/lib/utils";
import { sendWhatsAppDocumentTemplate } from "@/lib/whatsapp/outbound";
...
import { deliverTemplateOnly, type DeliveryResult } from "@/lib/whatsapp/session-window";
...
export async function sendWeeklyReport(input: {
  ...
  const result = await deliverTemplateOnly({
    ...
    onTemplateUnusable: async (err) => {
      console.warn(`[weekly-report] template "${template}" unusable (Meta ${err.code}) — no plain-text fallback will be sent.`);
