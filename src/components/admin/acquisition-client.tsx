...
export function AcquisitionClient({ data, canWrite }: { data: AcquisitionDashboardData; canWrite: boolean }) {
  ...
  async function create(form: HTMLFormElement) {
    ...
    body.nextActionAt = new Date(String(body.nextActionAt)).toISOString();
...
function Field({ name, label, required = false }: { name: string; label: string; required?: boolean }) { return <label className="text-xs text-vodium-cream/50">{label}<input name={name} required={required} className="input-dark w-full rounded-lg px-3 py-2 mt-1" /></label>; }
