  ...
  return (
    <div className="p-5 md:p-8 max-w-7xl mx-auto space-y-7">
      ...
      <section className="surface-card overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead className="bg-white/[0.025] text-left text-[10px] uppercase tracking-wider text-vodium-cream/35"><tr><th className="px-4 py-3">Merchant</th><th className="px-4 py-3">Contact</th><th className="px-4 py-3">Fit / stage</th><th className="px-4 py-3">Source</th><th className="px-4 py-3">Owner</th><th className="px-4 py-3">Next action</th></tr></thead><tbody>
          {visible.map((p) => <tr key={p.id} className="border-t border-white/[0.05] transition-colors hover:bg-white/[0.035]"><td className="px-4 py-4 align-top"><Link href={"/admin/acquisition/" + p.id} className="block max-w-[245px] truncate font-medium text-vodium-cream hover:text-vodium-gold">{p.businessName}</Link><p className="mt-1 max-w-[270px] truncate text-xs text-vodium-cream/35" title={p.locationText ?? undefined}>{p.community?.shortName ?? p.locationText ?? p.contactName ?? "No location"}</p></td><td className="px-4 py-4 align-top"><div className="space-y-1.5">{p.phone ? <a href={`tel:${p.phone}`} className="flex items-center gap-2 whitespace-nowrap text-xs text-vodium-cream/70 hover:text-vodium-gold"><Phone size={13} className="text-vodium-gold" />{p.phone}</a> : <span className="flex items-center gap-2 text-xs text-vodium-cream/30"><Phone size={13} />No phone</span>}{p.email ? <a href={`mailto:${p.email}`} className="flex max-w-[190px] items-center gap-2 truncate text-xs text-vodium-cream/50 hover:text-vodium-gold"><Mail size={13} className="shrink-0 text-vodium-gold" />{p.email}</a> : <span className="flex items-center gap-2 text-xs text-vodium-cream/30"><Mail size={13} />No email</span>}</div></td><td className="px-4 py-4 align-top"><span className="inline-flex rounded-full border border-vodium-gold/25 bg-vodium-gold/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-vodium-gold">{p.fit}</span><p className="mt-2 text-xs text-vodium-cream/45">{label(p.stage)}</p></td><td className="px-4 py-4 align-top text-xs text-vodium-cream/55">{SOURCE_LABEL[p.source]}</td><td className="px-4 py-4 align-top text-xs text-vodium-cream/55">{p.assignedTo?.name ?? "Unassigned"}</td><td className="px-4 py-4 align-top text-xs text-vodium-cream/55">{p.nextActionAt ? <><span>{p.nextActionType}</span><p className="mt-1 text-[11px] text-vodium-cream/35">{new Date(p.nextActionAt).toLocaleString("en-NG")}</p></> : "None"}</td></tr>)}
          {!visible.length && <tr><td colSpan={6} className="p-10 text-center text-vodium-cream/35">No prospects match this view.</td></tr>}
        </tbody></table></div>
      </section>
